        ...
        // when { branch "master" }
            ...
            steps {
                withCredentials([sshUserPrivateKey(credentialsId: "tacoSharedKey", keyFileVariable: "keyfile")]) {
                    ...
                    sh 'ssh -i ${keyfile} centos@34.216.218.113 sudo `which docker-compose` down && UI_IMAGE_NAME=084735579641.dkr.ecr.us-west-2.amazonaws.com/vws_taco_ui UI_IMAGE_TAG=latest API_IMAGE_NAME=084735579641.dkr.ecr.us-west-2.amazonaws.com/vws_taco_api API_IMAGE_TAG=latest sudo `which docker-compose` up -d'
                }
            }
        }
    }
